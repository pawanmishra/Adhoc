...
void Main()
{
	NumberedPairs("2,4,5,6,9,11,15;20");
}

private static void NumberedPairs(string line)
{
	List<string> list = new List<string>();
	var items = line.Split(';');
	var total = int.Parse(items[1]);
	var data = items[0].Trim().Split(',').Select(x => int.Parse(x)).ToList();
	...
	for(int i = 0; i < data.Count; i++)
	{
		...
		while(temp != data.Count - 1)
		{
			if(stack.Sum() == total)
			{
				var item = string.Join(",",stack.Select(x => x.ToString()).ToArray());
				list.Add(item);
				
				// pop all elements till j
				while(temp != j)
				{
					temp--;
					stack.Pop();
