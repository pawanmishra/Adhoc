...
//	foreach(Microsoft.SqlServer.Management.Smo.Database db1 in srv.Databases)
//	{
//		db1.Name.Dump();
//	}
	...
	foreach(Table tb in db.Tables) 
	{
		foreach (ForeignKey f in tb.ForeignKeys)
		{
			...
			Console.WriteLine("the foreign key refrences " + f.ReferencedTable + " table and column " + f.ReferencedKey + ".");
					
			foreach(ForeignKeyColumn data in f.Columns)
			{
				data.Name.Dump();
				data.ReferencedColumn.Dump();
			}
		}
	}
}
