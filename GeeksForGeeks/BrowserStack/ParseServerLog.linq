...
void Main()
{
	string filePath = @"C:\Users\mishrapa\Documents\LINQPad Queries\GeeksForGeeks\BrowserStack\ServerLog.txt";
	ParseServerLog(filePath);
}

public void ParseServerLog(string filePath)
{
	Dictionary<string, int> dict = new Dictionary<string, int>();
	Regex reg = new Regex(@"[HTP]+\/1.1.\s[0-9]{3}");
	using(TextReader reader = new StreamReader(filePath))
	{
		while(true)
		{
			...
			var code = value.Substring(value.Length - 3, 3);
