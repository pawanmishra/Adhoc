...
void Main()
{
	...
	int sum = 0;
	GetSum(n20, false, ref sum);
	sum.Dump();
	
	GetSumOfLeftLeaves(n20).Dump();
}
...
public static int GetSumOfLeftLeaves(Node node)
{
	int res = 0;
	...
	if(node != null)
	{
		if(IsLeafNode(node.Left))
			...
			res += GetSumOfLeftLeaves(node.Left);
			
		res += GetSumOfLeftLeaves(node.Right);
	}
	return res;
}

public static void GetSum(Node node, bool isLeftNode, ref int sum)
{
	if(node == null) return;
	if(node.Left == null && node.Right == null && isLeftNode)
		sum = sum + node.Value;
		
	GetSum(node.Left, true, ref sum);
	GetSum(node.Right, false, ref sum);
}
