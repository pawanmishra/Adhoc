<Query Kind="SQL">
  <Connection>
    <ID>7a085cc3-db0a-4cac-b7a6-c8131e3a0a75</ID>
    <Persist>true</Persist>
    <Server>.</Server>
    <Database>TimeTracker</Database>
...
select top(1) * from timetracker;
select top(1) * from timetrackerHistory;

;with test as (select t.ParentId as Id
from timetrackerHistory t
group by t.ParentId)
select distinct t.ParentId, t.Id, dense_rank() over(order by t.ParentId) as rank
from 
timetrackerHistory t inner join test tt
on t.ParentId = tt.Id;

;with tt as (select t.Id, t.ParentId, row_number() over(partition by t.ParentId order by t.Id) as rank
from timetrackerHistory as t)
select th.* from timetrackerHistory th
inner join tt on th.Id = tt.Id where tt.rank = 1;
select * from timetrackerHistory where ParentId = 23037;
select * from timetracker where id = 23037;

-- update parent table
;with tt as (select t.Id, t.ParentId, t.ActiveMinutes, row_number() over(partition by t.ParentId order by t.Id) as rank
from timetrackerHistory as t)
update t
set t.ActiveMinutes = t.ActiveMinutes - tt.ActiveMinutes
from timetracker t inner join tt on t.id = tt.ParentId

-- delete records from child table
;with tt as (select t.Id, t.ParentId, row_number() over(partition by t.ParentId order by t.Id) as rank
from timetrackerHistory as t)
delete th from timetrackerHistory th
inner join tt on th.Id = tt.Id where tt.rank = 1;
